...
def LOGO(Probability, figure_name, xi_ind=1, top_rank=20, figsize=(10, 2.5)):
    ...
    ww_logo.fig.savefig(figure_name)
    plt.clf()
    plt.cla()
...
#    target_res= '1,-1'
    ...
    f = open(result_file+'_prob.txt', 'w')
    f.writelines(item)
    f.close()
    ...
    if mode.startswith('BBO'):
        ...
        for i in range (num_fig):
            if i == num_fig-1:
                ...
                LOGO(Pred_prob, result_file+'_'+str(i+1)+'_logo.png', xi_ind=i*xtick_len+1, top_rank=6, figsize=(10*fig_p+1, 2.5))
                # eps format will not show the fade effection
            else:
                Pred_prob = prediction_prob[i*xtick_len : (i+1)*xtick_len]
                LOGO(Pred_prob, result_file+'_'+str(i+1)+'_logo.png', xi_ind=i*xtick_len+1, top_rank=6, figsize=(10, 2.5))

    else:
        BBO_figure(prediction_prob, result_file+'_logo.png')